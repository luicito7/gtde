        ...
        <span class="hidden sm:align-middle sm:h-screen " ></span>
            ...
            <div class="inline-block overflow-auto text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-10 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline" >    
                <span class="flex flex-row-reverse pt-2 pr-2">
                    <button wire:click.prevent="cerrarModalBAsociacion()" class="px-2 py-1 text-white transition duration-150 ease-in-out bg-red-600 border border-transparent rounded-full focus:outline-none focus:border-green-400 focus:shadow-outline-green sm:text-sm sm:leading-4">X</button>
                </span>
                <form>
                    
                    <div class="items-center px-4 pt-0 pb-4 bg-white-50 sm:p-6 sm:pb-4" >
                        <div class="flex items-center justify-between pb-5">
                            <h2>Buscar Asociación</h2>
                            <div class="relative self-center text-gray-600 sm:mb-0 focus-within:text-gray-400">
                                ...
                                <input wire:model="searchTerm2" type="search" name="q" class="py-2 pl-10 text-sm border-2 rounded-md focus:outline-none focus:bg-green focus:text-gray-900" placeholder="Buscar..." autocomplete="on"  pattern="[A-Z]{a-z}" wire:model="nombreasoc"  >
                        ...
                        <div class="flex justify-center flex-grow max-h-full mb-6 -mx-3">
                            <div >    
                                <table class="table-fixed w- full">
                                    ...
                                    <tbody>
                                        @foreach ($associations as $datos1)
                                        <tr class="h-20 text-xs">                                           
                                            <td class="px-4 py-2 border">{{$datos1->nombreasoc}}</td>
                                            <td class="px-4 py-2 border">{{$datos1->dnirepre}}</td>
                                            <td class="px-4 py-2 border">{{$datos1->dnideleg}}</td>
                                            <td class="px-4 py-2 border">{{$datos1->rubroasoc}}</td>
                                            <td>
                                                <button wire:click.prevent="almacenarInput1({{$datos1->id}})" class="inline-flex justify-center w-full px-4 py-2 text-white transition duration-150 ease-in-out bg-green-500 border border-transparent rounded-md focus:outline-none focus:border-green-500 focus:shadow-outline-green sm:text-xs sm:leading-5">Utilizar</button>
                                            </td>
